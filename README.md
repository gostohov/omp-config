...
Репозиторий содержит версионируемую часть конфигурации oh-my-pi.
...
Для chat-ролей и специализированных `tiny`, `memory`, `web`, `judge` заданы
явные пустые `retry.fallbackChains`. Поэтому при недоступности выбранной модели
OMP не подставляет встроенный fallback и не меняет локальный или корпоративный
маршрут незаметно.
...
Версия должна совпадать с указанной в разделе «Мета-информация». Режим
